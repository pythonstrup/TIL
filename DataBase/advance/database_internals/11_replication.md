...
- 동기화 = 높은 비용
- 따라서 어느 정도의 불일치를 허용하는 모델도 있다.

### 조정 가능한 일관성

- 일관성과 가용성이 대립 관계에 있는 시스템
1. `복제 팩터 N`: 데이터 복제본을 저장하는 노드 수
2. `쓰기 일관성 W`: 쓰기를 성공적으로 하기 위해 응답해야 하는 노드 수
3. `읽기 일관선 R`: 읽기를 성공적으로 하기 위해 응답해야 하는 노드 수

- `(R + W > N)`을 충족하는 일관성 수준은 읽기와 쓰기 작업 사이에 겹치는 노드가 항상 존재하기 때문에 최신 값을 반환 
- ex) `N=3, W=2, R=2`: 동시에 최대 한 개의 노드에만 장애가 발생할 수 있음.

### 증명 복제 노드

- 쿼럼 기반 읽기를 통해 시스템 가용성을 높일 수도 있다.
  - 일부 노드가 중단돼도 데이터베이스는 계속해서 읽기와 쓰기 요청을 처리할 수 있다.
- 저장 비용은 `감시 복제 노드 witness replica` 개념을 사용해 줄일 수 있다.
  - 각 복제 노드마다 복제본을 저장하는 대신 복제 노드를 카피 그룹과 `감시 witness` 그룹으로 나눈다.
  - 카피 노드에 장애 또는 쓰기 타임아웃이 발생하면 감시 노드를 카피 노드로 업그레이드하고 레코드를 임시 저장한다. 
  - 노드가 복구되면 감시 노드를 이전 상태로 되돌리거나 복구된 노드를 감시 노드로 사용한다.

### 강력한 결과적 일관성과 CRDTs

- 어느 정도의 비일관성을 허용. 일치하지 않는 상태는 수행 후에 조정할 수 있도록 연산마다 상태를 추가로 저장 => ex) Redis에 구현된 `충돌이 없는 복제된 데이터 타입 Conflict-Free Replicated Date Types`
- 충돌이 발생하지 않는 특수한 자료구조라..
  - 연산을 어떠한 순서로 수행해도 결과가 똑같다. 분산 시스템에서 굉장히 유용.
  - 네트워크 파티션이 발생해도, 연결이 복구되는 즉시 모든 노드의 결과를 병합할 수 있다.
- 일시적 불일치를 허용하기 때문에 결과적 일관성 기반 시스템에서 유용하다.
- `CRDTs`의 가장 간단한 예) `CmRDTs` (Commutative Replicated Data Types). 아래의 조건을 충족해야 한다.
1. 부가 효과 없음
2. 순서 무관
3. 인과 순서 존재

- 이 개념들은 장애를 허용하는 분산 시스템을 구축하는 데 큰 도움이 되는 중요한 개념.
