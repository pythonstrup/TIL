...
- (item45에서 언급) 스트림은 `반복 iteration`을 지원하지 않는다. 따라서 스트림과 반복을 알맞게 조합해야 좋은 코드가 나온다.
  - `Iterable`을 지원하지 않아 for문에서 사용이 불가능하다는 말인 걸로 이해 
  - 그리고 스트림은 1회성 소비만 가능하다는 점을 기억하자.
- 공개 API를 작성할 때는 스트림 파이프라인을 사용하는 사람과 반복문을 사용하려는 사람 모두를 배려해야 한다.
  - `Collection` 인터페이스는 `Iterable`의 하위 타입이고 `stream` 메소드도 제공하니 반복과 스트림을 동시에 지원한다.
  - 따라서 `Collection`의 하위 타입을 사용하는 것을 추천

---

## item48. 스트림 병렬화는 주의해서 적용하라

- `stream`에는 `parallel` 메소드가 있는데, 파이프라인을 병렬로 실행할 수 있다.
  - 동시성 프로그래밍을 할 때는 `안정성 safety`과 `응답 가능 liveness` 상태를 유지하기 위해 애써야 한다.
- 스트림 라이브러리가 파이프라인을 병렬화하는 방법을 찾아내지 못하면 오히려 성능이 엄청나게 안 좋아질 수 있다.
  - 환경이 아무리 좋더라도 데이터 소스가 `Stream.iterate`거나 중간 연산으로 `limit`을 사용하면 파이프라인 병렬화로는 성능 개선을 기대할 수 없다.
  - 특히, `limit`을 사용할 때 CPU 코어가 남는다면 원소를 몇 개 더 처리한 후 제한된 개수 이후의 결과를 버려도 아무런 해가 없다고 가정한다. => 만약 이전 결과를 만들어내는 비용이 높다면, 엄청난 손해다.
- 대체로 스트림의 소스사 `ArrayList`, `HashMap`, `HashSet`, `ConcurrentHashMap`의 인스턴스거나 배열, int 범위, long 범위일 때 병렬화의 효과가 가장 좋다.
  - 이 자료구조들은 모두 데이터를 원하는 크기로 정확하고 손쉽게 나눌 수 있어서 일을 다수의 스레드에 분배하기에 좋다는 특징이 있다.
  - 나누는 작업은 `Spliterator`가 담당하며, `Spliterator` 객체는 `Stream`이나 `Iterable`의 `spliterator` 메소드로 얻어올 수 있다.
  - 이 자료구조들의 또 다른 중요한 공통점. 원소들을 순차적으로 실행할 때 `참조 지역성 locality of reference`가 뛰어나다는 것이다. 이웃한 원소의 참조들이 메모리에 연속해서 저장되어 있다는 뜻이다.
  - 참조 지역성은 병렬 처리를 할 때 아주 중요한 요소로 작용한다.
- 스트림 파이프라인의 종단 연산의 동작 방식 역시 병렬 수행 효율에 영향을 준다.
  ...
  - 종단 연산 중 병렬화에 가장 적합한 것은 `축소 reduction`다. 축소는 파이프라인에서 만들어진 모든 원소를 하나로 합치는 작업으로 `reduce`, `min`, `max`, `count`, `sum` 같이 완성된 형태로 제공되는 메소드 중 하나를 선택해 수행
  - `anyMatch`, `allMatch`, `noneMatch`처럼 조건에 맞으면 바로 반환되는 메소드도 병렬화에 적합하다.
  - 반면, `가변 축소 mutable reduction`를 수행하는 `Stream`의 `collect` 메소드는 병렬화에 적합하지 않다. 컬렉션들을 합치는 부담이 크기 때문이다.
- 조건만 잘 갖춰지면 `parallel` 메소드를 호출해 거의 프로세서 코어 수에 비례하는 성능 향상을 할 수 있다.
